...
async function findNearestDriver(pickupLat, pickupLng, maxDistanceMeters = 5000) {
  const drivers = await sequelize.query(`
    ...
    FROM "Drivers"
    WHERE "isAvailable" = true AND location IS NOT NULL
    AND ST_DWithin(location::geography, ST_SetSRID(ST_MakePoint(:lng, :lat), 4326)::geography, :maxDistance)
    ORDER BY distance ASC LIMIT 1;
  `, { replacements: { lat: pickupLat, lng: pickupLng, maxDistance: maxDistanceMeters }, type: QueryTypes.SELECT });
  return drivers[0] || null;
}

module.exports = { Driver, updateDriverLocation, findNearestDriver };
